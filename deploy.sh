#!/bin/bash
# =============================================================================
# deploy.sh — Production VPS deployment for Planary Server
# =============================================================================
#
# DEFAULT: pull prebuilt API/worker image from GHCR and restart.
...
echo "Starting VPS deployment of Planary Server..."
...
read_env_var PLANARY_SERVER_IMAGE
read_env_var PORT

COMPOSE_FILE="docker-compose.vps.yml"
PROJECT="planary-server-vps"
DEPLOY_LOCK="${TMPDIR:-/tmp}/planary-server-deploy.lock"
DEFAULT_IMAGE="ghcr.io/somadina94/expenses-tracker-server:latest"
PLANARY_SERVER_IMAGE="${PLANARY_SERVER_IMAGE:-$DEFAULT_IMAGE}"
DEPLOY_BUILD_LOCAL="${DEPLOY_BUILD_LOCAL:-0}"
export PLANARY_SERVER_IMAGE
...
stop_own_stack() {
    echo "Stopping Planary Server stack (if running)..."
...
pull_image() {
    echo "Pulling prebuilt image: ${PLANARY_SERVER_IMAGE}"
    login_ghcr
    local attempt
    for attempt in 1 2 3; do
        if docker pull "${PLANARY_SERVER_IMAGE}"; then
            return 0
        fi
        echo "Pull failed (attempt ${attempt}/3), retrying in 15s..."
        sleep 15
    done
    echo "Failed to pull ${PLANARY_SERVER_IMAGE}"
...
build_image_local() {
    ...
    PLANARY_SERVER_IMAGE="${PROJECT}-api:latest"
    export PLANARY_SERVER_IMAGE
}
...
RUNNING="$($DOCKER_COMPOSE -p "$PROJECT" -f "$COMPOSE_FILE" ps --status running)"
if ! echo "$RUNNING" | grep -q "expenses-app"; then
    echo "Deployment failed: API container is not running"
    $DOCKER_COMPOSE -p "$PROJECT" -f "$COMPOSE_FILE" logs --tail=50
    exit 1
fi
if ! echo "$RUNNING" | grep -q "expenses-redis"; then
    echo "Deployment failed: redis container is not running"
    $DOCKER_COMPOSE -p "$PROJECT" -f "$COMPOSE_FILE" logs --tail=50
    exit 1
fi
if ! echo "$RUNNING" | grep -q "notifications-worker"; then
    echo "Deployment failed: notifications-worker container is not running"
    $DOCKER_COMPOSE -p "$PROJECT" -f "$COMPOSE_FILE" logs --tail=50
    exit 1
fi
